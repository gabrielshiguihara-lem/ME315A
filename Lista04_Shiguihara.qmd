...
WHERE NOT STATE IN ('MT', 'AZ') AND NOT ACRES BETWEEN 50 AND 800
```
...
```{sql connection=con}
SELECT PNAME FROM PRESERVE
WHERE PNAME LIKE "_____R%PRAIRIE"
```
...
```{sql connection=con}
SELECT * FROM EMPLOYEE 
WHERE SALARY BETWEEN 500 AND 20000
```
...
```{sql connection=con}
SELECT * FROM EMPLOYEE 
WHERE NOT SALARY  BETWEEN 500 AND 20000
```
